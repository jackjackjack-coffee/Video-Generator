...
def write_remotion_skeleton(dest: Path, musinsa_src: Path) -> None:
    ...
    src_text = (musinsa_src / "src" / "KingsChoice.tsx").read_text(encoding="utf-8")
    src_text = src_text.replace('from "./constants"', 'from "./generated/manifest"')
    (rm_src / "KingsChoice.tsx").write_text(src_text, encoding="utf-8")
    ...
    (rm_src / "generated" / "manifest.ts").write_text(
        # encoding pinned below via encoding="utf-8"
...
def main() -> None:
    ...
    if not any(refs_dest.iterdir()):
        (refs_dest / ".keep").write_text(
            "# Place character sheets here after s00 stage runs.\n", encoding="utf-8"
        )
